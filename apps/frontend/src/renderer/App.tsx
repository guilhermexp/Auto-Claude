...
export function App() {
  ...
  // Sync spell check language with i18n language (skip if unchanged)
  const lastSpellCheckLangRef = useRef<string | null>(null);
  useEffect(() => {
    const lang = i18n.language;
    if (lang === lastSpellCheckLangRef.current) return;
    lastSpellCheckLangRef.current = lang;

    const syncSpellCheck = async () => {
      try {
        const result = await window.electronAPI.setSpellCheckLanguages(lang);
        if (!result.success) {
  ...
  const handleOpenGitLabSettings = useCallback(() => {
            ...
            {selectedProject ? (
              <>
                ...
                {activeView === 'gitlab-issues' && (activeProjectId || selectedProjectId) && (
                  <GitLabIssues
                    onOpenSettings={handleOpenGitLabSettings}
                    onNavigateToTask={handleGoToTask}
                  />
                )}
