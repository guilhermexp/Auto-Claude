        ...
        <div className="grid gap-2 text-xs">
          ...
          {service.testing && (
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              <span>{t('serviceCard.testingLabel')}: {service.testing}</span>
            </div>
          )}
          {service.orm && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Database className="h-3 w-3 shrink-0" />
              <span>{t('serviceCard.ormLabel')}: {service.orm}</span>
            </div>
          )}
          {service.default_port && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Globe className="h-3 w-3 shrink-0" />
              <span>{t('serviceCard.portLabel')}: {service.default_port}</span>
            </div>
          )}
          {service.styling && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Code className="h-3 w-3 shrink-0" />
              <span>{t('serviceCard.stylingLabel')}: {service.styling}</span>
            </div>
          )}
          {service.state_management && (
            <div className="flex items-center gap-2 text-muted-foreground">
              <Package className="h-3 w-3 shrink-0" />
              <span>{t('serviceCard.stateLabel')}: {service.state_management}</span>
            </div>
          )}
        </div>
