...
export function MemoryCard({ memory }: MemoryCardProps) {
  ...
  const typeLabelKey = memoryTypeLabels[memory.type];
  const typeLabel = typeLabelKey ? t(typeLabelKey) : memory.type.replace(/_/g, ' ');

  const sessionLabel = memory.session_number
    ? t('memoryCard.sessionLabel', { count: memory.session_number })
    : parsed?.session_number
      ? t('memoryCard.sessionLabel', { count: parsed.session_number })
      : null;
