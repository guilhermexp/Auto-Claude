...
function DroppableStatusColumn({
  ...
  const columnLabel = t(`kanban.columns.${column.id}`, { defaultValue: column.label });
...
export function RoadmapKanbanView({
  ...
  const getStatusLabelForFeature = (feature: RoadmapFeature) => {
    ...
    return t(`kanban.columns.${feature.status}`, { defaultValue: statusColumn?.label || t('kanban.unknownStatus') });
  };
